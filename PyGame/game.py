...
x_cobra = int(largura/2) #colocando o objeto no meio da tela
...
def reiniciar_jogo():
    global conta_ponto, comprimento_inicial, x_cobra, y_cobra, lista_cobra, x_maca, y_maca, morreu
    conta_ponto = 0
    comprimento_inicial = 7
    x_cobra = int(largura / 2)
    y_cobra = int(altura / 2)
    lista_cobra.clear()  # Limpar a lista da cobra
    x_maca = randint(40, 600)
    y_maca = randint(50, 430)
    morreu = False
...
while True:# Loop principal do jogo
    relogio.tick(60)#ajusta a taxa de quadros que o jogo sera rodado
